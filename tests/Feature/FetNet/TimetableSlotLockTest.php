<?php

namespace Tests\Feature\FetNet;

use App\Models\FetNet\AcademicYear;
use App\Models\FetNet\Activity;
use App\Models\FetNet\ActivityPlanning;
use App\Models\FetNet\Client;
use App\Models\FetNet\Program;
use App\Models\FetNet\Semester;
use App\Models\FetNet\Subject;
use App\Models\FetNet\TimetableSlot;
use App\Models\User;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Livewire\Livewire;
use Tests\TestCase;

class TimetableSlotLockTest extends TestCase
{
    use RefreshDatabase;

    private const PAGE = 'pages::client.timetable.view.idx';

    /** @return array{0:User,1:Semester,2:TimetableSlot} */
    private function scaffold(): array
    {
        $user    = User::factory()->create();
        $client  = Client::create(['user_id' => $user->id]);
        $program = Program::create(['client_id' => $client->id, 'user_id' => $user->id, 'abbrev' => 'TE', 'name' => 'TE']);
        $ay      = AcademicYear::create(['client_id' => $client->id, 'year_start' => 2026, 'is_active' => true]);
        $sem     = Semester::create(['client_id' => $client->id, 'academic_year_id' => $ay->id, 'semester' => 1, 'name' => 'Ganjil']);
        $subject = Subject::create(['program_id' => $program->id, 'code' => 'EL1', 'name' => 'S', 'credit' => 3, 'semester' => 1]);
        $plan    = ActivityPlanning::create(['subject_id' => $subject->id, 'program_id' => $program->id, 'semester_id' => $sem->id]);
        $act     = Activity::create(['program_id' => $program->id, 'planning_id' => $plan->id, 'duration' => 1, 'active' => true]);

        $slot = TimetableSlot::create([
            'client_id'   => $client->id,
            'semester_id' => $sem->id,
            'activity_id' => $act->id,
            'day_index'   => 1,
            'hour_index'  => 1,
            'locked'      => false,
        ]);

        return [$user, $sem, $slot];
    }

    public function test_toggle_lock_flips_the_slot_lock_flag(): void
    {
        [$user, $sem, $slot] = $this->scaffold();

        $page = Livewire::actingAs($user)->test(self::PAGE, ['sem' => $sem->id])
            ->set('view', 'teacher')->set('mode', 'table');

        $page->call('toggleLock', $slot->id);
        $this->assertTrue($slot->fresh()->locked);

        $page->call('toggleLock', $slot->id);
        $this->assertFalse($slot->fresh()->locked);
    }

    public function test_cannot_lock_another_clients_slot(): void
    {
        [, $sem, $slot] = $this->scaffold();

        // A different client must not be able to lock the first client's slot.
        $otherUser = User::factory()->create();
        Client::create(['user_id' => $otherUser->id]);

        Livewire::actingAs($otherUser)->test(self::PAGE, ['sem' => $sem->id])
            ->call('toggleLock', $slot->id);

        $this->assertFalse($slot->fresh()->locked);
    }
}
