...
class TimetableSlotConflictTest extends TestCase
{
    use RefreshDatabase;

    private const PAGE = 'pages::client.timetable.view.idx';

    public function test_slot_flags_conflict_when_assigned_teacher_is_not_available_then(): void
    {
        ...
        $teacher = Teacher::create(['program_id' => $program->id, 'code' => 'AA', 'name' => 'Alice']);
        ...
        // No constraint yet -> no conflict.
        $page = Livewire::actingAs($user)->test(self::PAGE, ['sem' => $sem->id]);
        $this->assertArrayNotHasKey($slot->id, $page->instance()->conflicts());

        // Mark the teacher not-available exactly at the slot's day/hour -> conflict.
        TeacherTimeConstraint::create(['teacher_id' => $teacher->id, 'day' => 2, 'hour' => 3]);

        $page = Livewire::actingAs($user)->test(self::PAGE, ['sem' => $sem->id]);
        $this->assertArrayHasKey($slot->id, $page->instance()->conflicts());
    }
}
