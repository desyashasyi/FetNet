...
/**
 * Read-only published timetable for the program, teacher-centric. Shows the latest
 * PUBLISHED FetCompile's placed slots for this program in the chosen semester, as a
 * day×hour grid or a flat table, filterable by teacher. Uses HasProgramSemester for the
 * year/semester pickers and ClientConfig for day/hour labels.
 */
new #[Layout('layouts.program')] class extends Component
{
    use HasProgramSemester;

    #[Url] public string $view = 'teacher'; // grid | teacher | student | room
...
<div>
    <x-header title="Timetable — Teachers"
                            ...
                            <td class="border border-base-300 p-1.5 align-top">
                                @foreach($cell as $slot)
                                    <div class="bg-primary/10 border-l-4 border-primary rounded px-2 py-1.5 mb-1 leading-snug">
                                        ...
                                        @endif
                                        @if($slot->room)
                                            <div class="text-sm font-mono text-base-content/70 truncate" title="{{ $slot->room->code ?? $slot->room->name }}">
