...
/**
 * Read-only published timetable for the program, room-centric. Shows the latest PUBLISHED
 * FetCompile's placed slots for this program in the chosen semester, as a day×hour grid
 * or a flat table, filterable by room. Uses HasProgramSemester for the year/semester
 * pickers and ClientConfig for day/hour labels.
 */
new #[Layout('layouts.program')] class extends Component
{
    use HasProgramSemester;

    #[Url] public string $view = 'room'; // grid | teacher | student | room
    ...
    {
        if (! $this->clientId || ! $this->semesterId) return false;
        return FetCompile::where('client_id', $this->clientId)
    ...
    public function hourLabels(): array
    {
        ...
        if (empty($bookable)) {
            $out = [];
            for ($i = 1; $i <= 8; $i++) {
                $out[] = sprintf('%02d:00–%02d:00', 7 + $i - 1, 7 + $i);
            }
            return $out;
        }
    ...
    {
        if (! $this->isPublished || ! $this->programId) return collect();
        ...
        return TimetableSlot::with([
            ...
            ->whereHas('activity', fn($q) => $q->where('program_id', $this->programId))
            ->get();
    }
    ...
    public function roomOptions(): array
    {
        return collect($this->placedSlots)
            ->filter(fn($s) => $s->room)
            ->pluck('room')->unique('id')->sortBy('code')
...
<div>
    <x-header title="Timetable — Rooms"
              subtitle="{{ $this->isPublished ? 'Latest published timetable' : 'No published timetable yet' }}"
              separator>
        <x-slot:actions>
            <div class="join">
    ...
    <div class="flex flex-wrap items-center gap-3 mb-4">
        ...
        <x-choices single wire:model.live="academicYearId" :options="$academicYearOptions"
                   placeholder="Academic year" class="w-max min-w-44" />
        @endif
        @if(count($semesterOptions))
        <x-choices single wire:model.live="semesterId" :options="$semesterOptions"
                   placeholder="Semester" class="w-max min-w-44" />
        @endif
    ...
    @if(! $this->isPublished)
        <x-card>
            <div class="flex flex-col items-center py-16 text-base-content/40 gap-2">
                <x-icon name="o-lock-closed" class="w-12 h-12" />
                <p class="text-sm">Client has not published a timetable for this semester yet.</p>
            </div>
        </x-card>
    @elseif(count($this->placedSlots) === 0)
        <x-card>
            <div class="flex flex-col items-center py-16 text-base-content/40 gap-2">
                <x-icon name="o-archive-box" class="w-12 h-12" />
                <p class="text-sm">No activities placed for this program in the latest run.</p>
            </div>
        </x-card>
    @elseif($mode === 'table')
            ...
            <tbody>
                ...
                @forelse($this->flatSlots as $r)
                    ...
                    <tr class="{{ $zebra }} hover:bg-primary/5 {{ $firstOfDay && $prevDay !== null ? 'border-t-2 border-base-300' : '' }}">
                        <td class="text-sm font-semibold">{{ $firstOfDay ? $r['day'] : '' }}</td>
                ...
                @empty
                    <tr><td colspan="7" class="text-center text-sm text-base-content/40 py-6">
                        No slots in current filter. Pick a teacher, student group, or room first.
                    </td></tr>
                @endforelse
            </tbody>
        </table>
                            ...
                            <td class="border border-base-300 p-1.5 align-top">
                                @foreach($cell as $slot)
                                    <div class="bg-primary/10 border-l-4 border-primary rounded px-2 py-1.5 mb-1 leading-snug">
                                        <div class="font-bold text-sm">{{ $slot->activity?->planning?->subject?->code ?? '—' }}</div>
                                        @if($slot->room)
                                            <div class="text-sm font-mono text-base-content/80" title="{{ $slot->room->code ?? $slot->room->name }}">
                                        ...
                                        @if($slot->activity?->teachers->isNotEmpty())
                                            <div class="text-xs text-base-content/70 truncate" title="{{ $slot->activity->teachers->pluck('name')->implode(', ') }}">
